...
KAFKA_TOPIC = os.environ.get('KAFKA_TOPIC', 'dwd_precip_10m')
...
def rest_kafka_producer(omMsg):
    ...
    try:
        ...
        connection.request("POST", "/irmi/applikationen/kafka-producer/kafka.php?TOPIC=dwd_wind2_m10&VERSION=2.0", omMsg, headers)
...
def precipitation():       
        ...
        ftp.cwd('/climate_environment/CDC/observations_germany/climate/10_minutes/precipitation/now/')          
        ...
        for filename in ftp.nlst():
            #if filename.startswith("10minutenwerte_nieder_000"): # downlowd 2 or 3 Stations data only
            if filename.startswith("10minutenwerte_nieder_"):# download all stations data in Germany
                curr += 1
        ...
        for filename in ftp.nlst():
            if filename.startswith("zehn_now_rr_Beschreibung_"): # 10minutenwerte_SOLAR_
                curr += 1
        ...
        df_2 = pd.read_table("zehn_now_rr_Beschreibung_Stationen.txt", 
        ...
        columns = ['STATIONS_ID', 'MESS_DATUM', 'RWS_DAU_10', 'RWS_10', 'RWS_IND_10', 'eor',
             'geoBreite', 'geoLaenge']
