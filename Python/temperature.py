...
KAFKA_TOPIC = os.environ.get('KAFKA_TOPIC', 'dwd_airtemp_10m')
...
def rest_kafka_producer(omMsg):
    ...
    try:
        ...
        connection.request("POST", "/irmi/applikationen/kafka-producer/kafka.php?TOPIC=dwd_airtemp_10m&VERSION=2.0", omMsg, headers)
...
def temperature():       
        ...
        ftp.cwd('climate_environment/CDC/observations_germany/climate/10_minutes/air_temperature/now/')          
        ...
        for filename in ftp.nlst():
            #if filename.startswith("10minutenwerte_TU_000"): # downlowd 2 or 3 Stations data only 
            if filename.startswith("10minutenwerte_TU_"): # download all stations data in Germany
                curr += 1
        ...
        for filename in ftp.nlst():
            if filename.startswith("zehn_now_tu_Beschreibung_"): 
                curr += 1
        ...
        df_2 = pd.read_table("zehn_now_tu_Beschreibung_Stationen.txt", 
        ...
        columns = ['STATIONS_ID', 'MESS_DATUM', 'PP_10', 'TT_10', 'TM5_10', 'RF_10', 'TD_10', 'eor',
