...
KAFKA_TOPIC = os.environ.get('KAFKA_TOPIC', 'dwd_solar_10m')
...
def rest_kafka_producer(omMsg):
    ...
    try:
        ...
        connection.request("POST", "/irmi/applikationen/kafka-producer/kafka.php?TOPIC=dwd_solar_m10&VERSION=2.0", omMsg, headers)
...
def wind():       
        ...
        ftp.cwd('climate_environment/CDC/observations_germany/climate/10_minutes/solar/now/')          
        ...
        for filename in ftp.nlst():
            #if filename.startswith("10minutenwerte_SOLAR_000"): # downlowd 2 or 3 Stations data only
            if filename.startswith("10minutenwerte_SOLAR_"):  # download all stations data in Germany
                curr += 1
        ...
        for filename in ftp.nlst():
            if filename.startswith("zehn_now_sd_Beschreibung_"): # 10minutenwerte_SOLAR_
                curr += 1
        ...
        df_2 = pd.read_table("zehn_now_sd_Beschreibung_Stationen.txt", 
        ...
        columns = ['STATIONS_ID', 'MESS_DATUM', 'DS_10', 'GS_10', 'SD_10', 'LS_10', 'eor',
                   'geoBreite', 'geoLaenge']  # original
